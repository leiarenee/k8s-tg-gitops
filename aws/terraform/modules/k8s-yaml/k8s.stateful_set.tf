resource "kubernetes_stateful_set" "instance" { 
  depends_on = [null_resource.module_depends_on]
  for_each = local.stateful_set.applications
  ...
  dynamic "spec" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
    ...
    content {
      pod_management_policy = lookup(spec.value, "podManagementPolicy", null)
      # Type: string   Optional Computed 
      ...
      service_name = lookup(spec.value, "serviceName", null)
      ...
      dynamic "update_strategy" { # Nesting Mode: list  
        for_each = lookup(spec.value, "updateStrategy", {})

        content {
          type = lookup(update_strategy.value, "type", null)
          # Type: string   Optional  

          dynamic "rolling_update" { # Nesting Mode: list  
            for_each = lookup(update_strategy.value, "rollingUpdate", {})
      ...
      dynamic "volume_claim_template" { # Nesting Mode: list  
        for_each = lookup(spec.value, "volumeClaimTemplates", {})

        content {
          dynamic "metadata" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
            for_each = contains(keys(volume_claim_template.value), "metadata") ? {item = volume_claim_template.value["metadata"]} : {}
          ...
          dynamic "spec" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
            for_each = contains(keys(volume_claim_template.value), "spec") ? {item = volume_claim_template.value["spec"]} : {}
