resource "kubernetes_deployment_v1" "instance" { 
  depends_on = [null_resource.module_depends_on]
  for_each = local.deployment_v1.applications
  ...
  dynamic "spec" { # Nesting Mode: list  Min Items : 1  Max Items : 1  
    ...
    content {
      ...
      progress_deadline_seconds = lookup(spec.value, "progressDeadlineSeconds", null)
      # Type: number   Optional  
      ...
      dynamic "strategy" { # Nesting Mode: list  Max Items : 1  
        ...
        content {
          ...
          dynamic "rolling_update" { # Nesting Mode: list  Max Items : 1  
            ...
            content {
              max_surge = lookup(rolling_update.value, "maxSurge", null)
              # Type: string   Optional  
